...
function Watches() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const { data: response } = await axios.get('http://rbshop.pythonanywhere.com/brand?filter=3')
                ...
                {data && data.map((product, index) => (
                    <motion.div
                        ...
                        <div className='flex flex-col sm:flex-row sm:items-center justify-between my-2'>
                            ...
                            {loading ? <Skeleton width='20%' height={25} /> : <button className="mt-1 py-1 sm:py-2 sm:px-3 text-sm font-medium text-center text-black border-2  hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-gray-200 dark:hover:bg-gray-300 dark:text-gray-700" onClick={() => { addCartOne(product); notify() }}>Sebede gosmak</button>}
                        </div>
