...
function Computer() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const { data: response } = await axios.get('http://rbshop.pythonanywhere.com/brand?filter=1')
    ...
    return (
        <div className='container mx-auto sm:w-full'>
            ...
            <h1 className='my-2 text-2xl font-bold sm:px-0 px-2'>Kompyuterler</h1>
            <div className='pb-5 px-2 sm:px-0 w-full grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
                {data && data.map((product, index) => (
                    <motion.div
                        ...
                        initial='hidden'
                        animate='visible'
                        key={index} className="bg-white w-full p-2 py-4 sm:p-5 flex flex-col justify-between max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div className='flex flex-col sm:flex-row sm:items-center justify-between my-2'>
                            ...
                            {loading ? <Skeleton width='20%' height={25} /> : <button className="mt-1 py-1 sm:py-2 sm:px-3 text-sm font-medium text-center border-2 text-black hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-gray-200 dark:hover:bg-gray-300 dark:text-gray-700" onClick={() => { addCartOne(product); notify() }}>Sebede gosmak</button>}
                        </div>
                        {loading ? <Skeleton width='100%' height={25} /> : <button onClick={() => openModal({ product })} className="mt:1 sm:mt-5 inline-flex justify-center items-center py-1 sm:py-2 sm:px-3 text-sm font-medium text-center text-black border-2 hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-gray-200 dark:hover:bg-gray-300 dark:text-gray-700">
                            Doly okamak
